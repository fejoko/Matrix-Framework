...
void matrix_statemanager_init(Matrix_Statemanager* const statemanager)
{
	...
	{
		MTRX_CORE_LOG("statemanager: initialisation", MATRIX_LOGGER_LEVEL_INFO, statemanager->data->logger);
	}
}
...
void matrix_statemanager_update(Matrix_Statemanager* const statemanager)
{
	...
	else
	{
		...
		{
			matrix_statemanager_log("statemanager: %s_state update", statemanager->state_index_top, statemanager);
...
void matrix_statemanager_draw2d(Matrix_Statemanager* const statemanager)
{
	...
	else
	{
		...
		{
			matrix_statemanager_log("statemanager: %s_state draw2d", statemanager->state_index_top, statemanager);
...
void matrix_statemanager_draw3d(Matrix_Statemanager* const statemanager)
{
	...
	else
	{
		...
		{
			matrix_statemanager_log("statemanager: %s_state draw3d", statemanager->state_index_top, statemanager);
	...
	else
	{
		if (strcmp("", MTRX_VECTOR_AT_AS(Matrix_Statemanager_State, 0, statemanager->states_vec_all).core.name))
		{
			...
			else
			{
				...
				{
					matrix_statemanager_log("statemanager: %s_state load", 0, statemanager);
			...
			else
			{
				...
				{
					matrix_statemanager_log("statemanager: %s_state enter", 0, statemanager);
		...
		else
		{
			...
			matrix_statemanager_log("statemanager: %s_state leave", 0, statemanager);
		}
		...
		for (size_t i = matrix_vector_capacity(statemanager->states_vec_all); i > 0; i--)
		{
			if (MTRX_VECTOR_AT_AS(Matrix_Statemanager_State, i - 1, statemanager->states_vec_all).is_loadet)
			{
				...
				else
				{
					...
					matrix_statemanager_log("statemanager: %s_state unload", i - 1, statemanager);
				}
		...
		for (size_t i = matrix_vector_capacity(statemanager->states_vec_all); i > 0; i--)
		{
			...
			else
			{
				...
				matrix_statemanager_log("statemanager: %s_state destruction", i - 1, statemanager);
			}
		...
		MTRX_CORE_LOG("statemanager: shutdown", MATRIX_LOGGER_LEVEL_INFO, statemanager->data->logger);
	}
}
